...
int countContains(const double a1[], int n1, const double a2[], int n2)
{
    ...
    if(n1<=0||n2<0||n1<n2)
        return 0;
    int nCount=countContains(a1+1, n1-1, a2, n2);
    if (a1[0]==a2[0])
        return countContains(a1+1, n1-1, a2+1, n2-1)+nCount;
    else
        return nCount;
}
...
int main()
{
    ...
    double array2[]={10,50,40,20,50,40,350};
    std::cout<<countContains(array, 7, array2, 8)<<std::endl;
    double array3[]={10,2,11,223,3,11,1,2};
    order(array3, 4);
    for (int i=0;i<4;i++)
