...
void Pit::doSomething()
{
    ...
    if(randInt(0, 49)==0)
    {
        ...
        switch(typesConsidered[randInt(0, numTypes)])
        {
            case 0:
            {
                RegularSalmonella *rs=new RegularSalmonella(getX(),getY(),world());
                world()->addActor(rs);
                m_salmonellaLeft--;
                if(m_salmonellaLeft==0)
                    typesLeft[0]=false;
                //salmonella
            ...
            case 1:
            {
                AggressiveSalmonella *as=new AggressiveSalmonella(getX(),getY(),world());
                world()->addActor(as);
                m_aggrSalmonellaLeft--;
                if(m_aggrSalmonellaLeft==0)
                    typesLeft[1]=false;
                //aggr salmonella
                world()->playSound(SOUND_BACTERIUM_BORN);
                break;
            ...
            case 2:
            {
                EColi *e=new EColi(getX(),getY(),world());
                world()->addActor(e);
                m_ecoliLeft--;
                if(m_ecoliLeft==0)
                    typesLeft[2]=false;
...
void Salmonella::doSomething()
{
    if(planDistance()>0)
    {
        ...
        double newX = (getX() + 3 * cos((angle*1.0)/180 * PI));
        double newY = (getY() + 3* sin((angle*1.0)/180 * PI));
